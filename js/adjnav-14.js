...
function adj_nav_show_products(transport) {
    Event.fire(document, 'adjnav:ajax_on_success', {"test": "TEST"});
    ...
    if (resp.products) {
        ...
        var navigation = $('adj-nav-navigation');
        if (navigation) {
            navigation.update(resp.layer);
        }

        $('adj-nav-ajax').value = ajaxUrl;
    }

    var plc = Element.down(document, '.content');

    // Inserting category image from response
    if (resp.image && plc) {
        Element.insert(plc, {top: resp.image});
    }

    // Cleaning up slider(s)
    $$('.awis-block').invoke('remove');

    // Inserting slider from response
    if (resp.slider && plc) {
        Element.insert(plc, {top: resp.slider});
    }
    ...
    // Re-bind event listeners after AJAX
    nekloInitNavigationFilters();
}
...
function neklo_adj_nav_make_request() {
    ...
    new Ajax.Request($('adj-nav-ajax').value + '?' + $('adj-nav-params').value + '&no_cache=true' + adj_nav_get_aitanswer(),
        {
            method: 'get',
            onSuccess: adj_nav_show_products
        }
    );
}

function send_google_request() {
    var params_string = $('adj-nav-params').value + '&',
        regexp_cat = 'cat=(.*?)&',
        regexp_designer = 'manufacturer=(.*?)&',
        regexp_size = 'size=(.*?)&',
        regexp_color = 'style_colors=(.*?)&',
        cat_id = params_string.match(regexp_cat),
        manufacturer_ids = params_string.match(regexp_designer),
        size_ids = params_string.match(regexp_size),
        color_ids = params_string.match(regexp_color);

    if (cat_id) {
        // Navigation User Usage
        var navigation = $$('span#cat-' + cat_id[1]);
        if (!Mage.Cookies.get(cat_id) && navigation) {
            Mage.Cookies.set(cat_id, true);
            if (!!(window && window.ga)) {
                ga('send', 'pageview', {'dimension2': navigation.first().innerHTML});
            }
        }
    }

    if (manufacturer_ids) {
        // Nav Filter - Designers
        var manufacturers = manufacturer_ids[1].split('-');
        manufacturers.forEach(function (element) {
            var manufacturer = $$('span#manufacturer-' + element);
            if (manufacturer.first()) {
                if (!!(window && window.ga)) {
                    ga('send', 'event', 'Left Nav Filter - Designers', 'Select', manufacturer.first().innerHTML);
                }
            }
        });
    }
    ...
    if (size_ids) {
        // Nav Filter - Size
        var sizes = size_ids[1].split('-');
        sizes.forEach(function (element) {
            var size = $$('a#size-' + element);
            if (size.first()) {
                if (!!(window && window.ga)) {
                    ga('send', 'event', 'Left Nav Filter - Size', 'Select', size.first().innerHTML);
                }
            }
        });
    }
    ...
    if (color_ids) {
        // Nav Filter - Color
        var colors = color_ids[1].split('-');
        colors.forEach(function (element) {
            var color = $$('span#style_colors-' + element);
            if (color.first()) {
                if (!!(window && window.ga)) {
                    ga('send', 'event', 'Left Nav Filter - Color', 'Select', color.first().readAttribute('data'));
                }
            }
        });
    }
...
function bind_filter(el, className, isSingleVal) {
    //only one  price-range can be selected
    var id = el.id || el.selectedOptions[0].id;

    if (isSingleVal) {
        var items = $('narrow-by-list').getElementsByClassName(className);
        var i, n = items.length;
        for (i = 0; i < n; ++i) {
            if (items[i].hasClassName(sel) && items[i].id != id)
    ...
    adj_nav_add_params(id.substr(0, pos), id.substr(pos + 1), isSingleVal);
    adj_nav_make_request();
}

function adj_update_links(evt, className, isSingleVal) {
    var link = Event.findElement(evt, 'INPUT'),
        sel = className + '-selected';

    if (!link) {
        link = Event.findElement(evt, 'A');
    }

    if (!link) {
        link = Event.findElement(evt, 'SPAN');
    }

    if (!link) {
        link = Event.findElement(evt, 'OPTION');
    }

    if (!link) {
        link = Event.findElement(evt, 'SELECT');
    }
    ...
    if (link.hasClassName(sel)) {
        ...
        link.removeClassName('active'); // for size
    }
    else {
        link.checked = true;
        link.addClassName(sel);
        link.addClassName('active');    // for size
    }

    bind_filter(link, className, isSingleVal);
    Event.stop(evt);
}
...
function adj_nav_category_listener(evt) {
    neklo_uncheck_checkboxes('.adj-nav-category', evt.target.id);
...
function neklo_uncheck_checkboxes(cbSelector, exceptId) {
    $$(cbSelector).each(function (i) {
        if (i.id != exceptId) {
            i.checked = false;
        }
    });
...
function adj_nav_toolbar_make_request(href) {
    ...
    neklo_adj_nav_make_request();
}

function neklo_sorter_adj_nav_toolbar_make_request(href) {
...
function bind_select(el) {
    ...
    for (i = 0; i < length; i++) {
        if (el.options[i].selected) {
            bind_filter(el.options[i], null, true);
            return;
        }
    }
    ...
    if (!el || !el.tagName || ('OPTION' != el.tagName)) {
        return;
    }
    var select = Element.up(el, 'select');
    if (!select) {
        return;
    }
    if (select.binded) {
    ...
    select.binded = true;
    Event.observe(select, 'change', function (evt) {
        var i = 0,
            length = this.options.length;

        for (i = 0; i < length; i++) {
            if (this.options[i].selected) {
                bind_filter(this.options[i], null, true);
                return;
            }
        }
        ...
        for (i = 0; i < n; ++i) {
            if (ajdnavExpandedLoaded || addOther == items[i].hasClassName('other')) { //ajdnavExpandedLoaded || false == false || true == true
                if(items[i].tagName === "SELECT") {
                    Event.observe(items[i], 'change', handler);
                } else {
...
function adjnavInitFeaturedAttributes() {
    ...
    function observeMoreAttributes() {
        if (!adjnavExpandedAttributes) {
            ...
            $$('#narrow-by-list dt.adjnav-attr-other').each(function (el) {
                el.show();
            });
            $$('#narrow-by-list dd.adjnav-attr-other').each(function (el) {
                el.show();
            });
...
function adj_nav_page_autoload_products_insert(el, mode) {
    ...
    if (mode == 'grid') {
        ...
        decorateGeneric($$('ul.categoryProductsMobile__grid'), ['odd','even','first','last']);
    }
}
